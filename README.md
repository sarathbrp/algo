...
- **Inverse / bear ETFs** (`universe.bear_etfs`): Separate from `universe.symbols`. As committed: **`symbols: [SQQQ, SPXS]`**, **`max_positions`** (distinct inverse tickers with exposure), **`max_exposure_pct_equity: 30`**, **`breakdown.reference_symbol: QQQ`**, **`ma_period: 50`**. When the breakdown reference is **QQQ**, **`prefer_sqqq_when_breakdown_reference_is_qqq: true`** (default) limits **new** inverse entries to **SQQQ** only (SPXS is not considered in that case). **`controlled_scaling`** (optional): **QQQ vs 50D MA only** in the live loop; finds **`active_step_idx`** as the last `steps[]` tier with `dist_pct >= reference_ma_distance_pct_min`, compares tracker **`scale_count`** (default **1** when in position and key missing) to **`active_step_idx`**, uses **`steps[scale_count].size_multiplier`** for the add, **`cooldown_minutes`** vs **`last_scale_ts`**, optional **`require_price_above_last_entry`** vs **`last_entry_price`**, and persists **`scale_count`**, **`last_entry_price`**, **`last_scale_ts`** on success. Exposure cap still applies. Legacy **`sqqq_add_on_strong_trend`** is **not** wired in the loop anymore (use **`controlled_scaling`** only).
...
| **`bear_etfs`** | `symbols`, `breakdown`, `prefer_sqqq_when_breakdown_reference_is_qqq`, `max_positions`, `max_exposure_pct_equity`, `stop_pct`, `controlled_scaling` |
